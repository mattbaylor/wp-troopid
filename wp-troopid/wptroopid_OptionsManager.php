...
class troopid_OptionsManager {
    ...
    public function settingsPage() {
        if (!current_user_can('manage_options')) {
            wp_die(__('You do not have sufficient permissions to access this page.', 'wordpress-troopid-plugin'));
        }
        ...
        ?>
        <div class="wrap">
            <h2><?php _e('System Settings', 'wordpress-idme-plugin'); ?></h2>
            <table class="form-table"><tbody>
            <tr><td><?php _e('System', 'wordpress-idme-plugin'); ?></td><td><?php echo php_uname(); ?></td></tr>
            <tr><td><?php _e('PHP Version', 'wordpress-idme-plugin'); ?></td>
                <td><?php echo phpversion(); ?>
                <?php
                if (version_compare('5.2', phpversion()) > 0) {
                    echo '&nbsp;&nbsp;&nbsp;<span style="background-color: #ffcc00;">';
                    _e('(WARNING: This plugin may not work properly with versions earlier than PHP 5.2)', 'wordpress-idme-plugin');
                    echo '</span>';
                }
                ?>
                </td>
            </tr>
            <tr><td><?php _e('MySQL Version', 'wordpress-idme-plugin'); ?></td>
                <td><?php echo $this->getMySqlVersion() ?>
                    <?php
                    echo '&nbsp;&nbsp;&nbsp;<span style="background-color: #ffcc00;">';
                    if (version_compare('5.0', $this->getMySqlVersion()) > 0) {
                        _e('(WARNING: This plugin may not work properly with versions earlier than MySQL 5.0)', 'wordpress-idme-plugin');
                    }
                    echo '</span>';
                    ?>
                </td>
            ...
            <h2><?php echo $this->getPluginDisplayName(); echo ' '; _e('Settings', 'wordpress-idme-plugin'); ?></h2>
            ...
            <?php settings_fields($settingsGroup); ?>
                ...
                <p class="submit">
                    <input type="submit" class="button-primary"
                           value="<?php _e('Save Changes', 'wordpress-idme-plugin') ?>"/>
                </p>
            </form>
        </div>
        <?php
    ...
    protected function getOptionValueI18nString($optionValue) {
        switch ($optionValue) {
            case 'true':
                return __('true', 'wordpress-troopid-plugin');
            case 'false':
                return __('false', 'wordpress-troopid-plugin');

            case 'Administrator':
                return __('Administrator', 'wordpress-troopid-plugin');
            case 'Editor':
                return __('Editor', 'wordpress-troopid-plugin');
            case 'Author':
                return __('Author', 'wordpress-troopid-plugin');
            case 'Contributor':
                return __('Contributor', 'wordpress-troopid-plugin');
            case 'Subscriber':
                return __('Subscriber', 'wordpress-troopid-plugin');
            case 'Anyone':
                return __('Anyone', 'wordpress-trooopid-plugin');
        }
        return $optionValue;
    }
